...
float const DEG2RAD_CONSTANT = M_PI / 180.0f;
float const RAD2DEG_CONSTANT = 180.0f / M_PI;
...
float const MIN_NATURAL_ROTATION_RAD_PER_SEC = 3.14f;  // Approx 0.5 RPS (base spin for any throw)
float const THROW_VELOCITY_TO_ROTATION_SCALE = 2.5f;  // How much throw speed (m/s) contributes to rotation speed (rad/s).
float const MAX_NATURAL_ROTATION_FROM_VELOCITY_RAD_PER_SEC = 70.0f;  // Cap to prevent insane spins from velocity
float const MIN_THROW_SPEED_FOR_CROSS_PRODUCT_ROTATION = 0.2f;  // Min throw speed (m/s) to attempt cross-product rotation, else min spin.
...
GlobalNamespace::OVRInput::Button GetOVRButtonForConfig(int configuredButtonIndex, bool isLeftController) {
    if (configuredButtonIndex == 0) {
        return GlobalNamespace::OVRInput::Button::None;
    }
    if (isLeftController) {
        switch (configuredButtonIndex) {
            case 1:
                return GlobalNamespace::OVRInput::Button::One;
            case 2:
                return GlobalNamespace::OVRInput::Button::Two;
            case 3:
                return GlobalNamespace::OVRInput::Button::PrimaryIndexTrigger;
            case 4:
                return GlobalNamespace::OVRInput::Button::PrimaryHandTrigger;
            default:
                return GlobalNamespace::OVRInput::Button::None;
        }
...
MAKE_HOOK_MATCH(
    MainMenuViewController_DidActivate_Hook,
...
MAKE_HOOK_MATCH(
    SaberModelController_Init_Hook,
    ...
    if (!saber) {
        getLogger().error("[TS] [SMC] Saber is null");
        return;
    }
    UnityEngine::Transform* currentSaberActualTransform = saber->get_transform();
    if (!currentSaberActualTransform) {
        getLogger().error("[TS] [SMC] Saber transform is null");
        return;
    }
...
MAKE_HOOK_MATCH(TrickSaberInputUpdateHook, &GlobalNamespace::OculusVRHelper::Update, void, GlobalNamespace::OculusVRHelper* self) {
    ...
    if (leftHandTransform) {
        leftControllerVelocity =
            UnityEngine::Vector3::op_Division(UnityEngine::Vector3::op_Subtraction(leftHandTransform->get_position(), prevLeftHandPos), deltaTime);
        prevLeftHandPos = leftHandTransform->get_position();
    }
    if (rightHandTransform) {
        rightControllerVelocity =
            UnityEngine::Vector3::op_Division(UnityEngine::Vector3::op_Subtraction(rightHandTransform->get_position(), prevRightHandPos), deltaTime);
    ...
    if (leftSaberTransform && leftHandTransform && leftSaberOriginalParent) {
        ...
        if (leftThrowBtnIdx > 0) {
            leftThrowInputPressed =
                GlobalNamespace::OVRInput::Get(GetOVRButtonForConfig(leftThrowBtnIdx, true), GlobalNamespace::OVRInput::Controller::LTouch);
        }
        ...
        if (leftThrowInputPressed && leftSaberState == SaberInteractionState::Held && !leftThrowButtonPressedLastFrame) {
            ...
            if (leftSpinActive) {
                ...
                leftSaberSimulatedWorldAngularVelocity =
                    UnityEngine::Vector3::op_Multiply(leftSaberTransform->TransformDirection(localSpinAxis), direction * speed * DEG2RAD_CONSTANT);
            } else {
                ...
                if (throwSpeedMagnitude < 1.0f) {  // Gentle throw -> Minimal
                    UnityEngine::Vector3 worldRollAxis = leftSaberTransform->TransformDirection(UnityEngine::Vector3::get_forward());
                    leftSaberSimulatedWorldAngularVelocity =
                        UnityEngine::Vector3::op_Multiply(worldRollAxis, throwSpeedMagnitude * MIN_NATURAL_ROTATION_RAD_PER_SEC);
                    ...
                    UnityEngine::Vector3 saberForwardWorld = leftSaberTransform->TransformDirection(UnityEngine::Vector3::get_forward());

                    // Cross product to get a perpendicular spin axis
                    UnityEngine::Vector3 naturalSpinAxisWorld = UnityEngine::Vector3::Cross(saberForwardWorld, throwDirectionWorldNormalized);
                    ...
                    leftSaberSimulatedWorldAngularVelocity = UnityEngine::Vector3::op_Multiply(naturalSpinAxisWorld, totalNaturalSpinRadPerSec);
                }
            }
            leftSpinActive = false;
        ...
        if (leftSpinBtnIdx > 0 && leftSaberState == SaberInteractionState::Held) {
            bool spinInputPressed =
                GlobalNamespace::OVRInput::Get(GetOVRButtonForConfig(leftSpinBtnIdx, true), GlobalNamespace::OVRInput::Controller::LTouch);
            if (spinInputPressed && !leftSpinActive) {
                leftSpinActive = true;
                getLogger().info("[TS] [FixedUpdate] [Left Saber] Spin Activated");
            } else if (!spinInputPressed && leftSpinActive) {
                leftSpinActive = false;
                getLogger().info("[TS] [FixedUpdate] [Left Saber] Spin Deactivated, restoring position");
                if (leftSaberTransform) {
                    leftSaberTransform->set_localRotation(originalLeftSaberLocalRotation);
                    leftSaberTransform->set_localPosition(originalLeftSaberLocalPosition);
                }
        ...
        if (leftSaberState == SaberInteractionState::Held) {
            ...
            UnityEngine::Vector3 newPos =
                UnityEngine::Vector3::op_Addition(currentPos, UnityEngine::Vector3::op_Multiply(leftSaberSimulatedWorldVelocity, deltaTime));
            ...
            if (leftSaberSimulatedWorldAngularVelocity.get_sqrMagnitude() > 0.0001f) {
                ...
                leftSaberTransform->set_rotation(UnityEngine::Quaternion::op_Multiply(deltaRotation, leftSaberTransform->get_rotation()));
            }
            ...
            if (leftSaberReturnDuration < 0.01f) {
                leftSaberReturnDuration = 0.01f;
            }
            ...
            UnityEngine::Quaternion targetRot_world =
                UnityEngine::Quaternion::op_Multiply(leftHandTransform->get_rotation(), originalLeftSaberLocalRotation);
            ...
            if (leftSaberSimulatedWorldAngularVelocity.get_sqrMagnitude() > 0.0001f && t < 0.95f) {
                ...
                UnityEngine::Quaternion deltaRotation = UnityEngine::Quaternion::AngleAxis(angle, axis);
                leftSaberTransform->set_rotation(UnityEngine::Quaternion::op_Multiply(deltaRotation, leftSaberTransform->get_rotation()));
                leftSaberTransform->set_rotation(UnityEngine::Quaternion::Slerp(leftSaberTransform->get_rotation(), targetRot_world, t * t * t));
            } else {
                leftSaberTransform->set_rotation(UnityEngine::Quaternion::Slerp(leftSaberThrowReleaseRotation, targetRot_world, t));
            }
    ...
    if (rightSaberTransform && rightHandTransform && rightSaberOriginalParent) {
        ...
        if (rightThrowBtnIdx > 0) {
            rightThrowInputPressed =
                GlobalNamespace::OVRInput::Get(GetOVRButtonForConfig(rightThrowBtnIdx, false), GlobalNamespace::OVRInput::Controller::RTouch);
        }
        ...
        if (rightThrowInputPressed && rightSaberState == SaberInteractionState::Held && !rightThrowButtonPressedLastFrame) {
            ...
            if (rightSpinActive) {  // If it was being actively spun by your mod
                ...
                float direction = clockwise ? 1.0f : -1.0f;
                UnityEngine::Vector3 localSpinAxis = UnityEngine::Vector3::get_right();
                rightSaberSimulatedWorldAngularVelocity =
                    UnityEngine::Vector3::op_Multiply(rightSaberTransform->TransformDirection(localSpinAxis), direction * speed * DEG2RAD_CONSTANT);
            } else {
                ...
                if (throwSpeedMagnitude < 1.0f) {
                    ...
                    UnityEngine::Vector3 throwDirectionWorldNormalized = throwVelocityWorld.get_normalized();
                    UnityEngine::Vector3 saberForwardWorld = rightSaberTransform->TransformDirection(UnityEngine::Vector3::get_forward());
                    ...
                    rightSaberSimulatedWorldAngularVelocity = UnityEngine::Vector3::op_Multiply(naturalSpinAxisWorld, totalNaturalSpinRadPerSec);
                }
            }
            rightSpinActive = false;
        ...
        if (rightSpinBtnIdx > 0 && rightSaberState == SaberInteractionState::Held) {
            bool spinInputPressed =
                GlobalNamespace::OVRInput::Get(GetOVRButtonForConfig(rightSpinBtnIdx, false), GlobalNamespace::OVRInput::Controller::RTouch);
        ...
        if (rightSaberState == SaberInteractionState::Held) {
            ...
            UnityEngine::Vector3 newPos =
                UnityEngine::Vector3::op_Addition(currentPos, UnityEngine::Vector3::op_Multiply(rightSaberSimulatedWorldVelocity, deltaTime));
            ...
            if (rightSaberSimulatedWorldAngularVelocity.get_sqrMagnitude() > 0.0001f) {
                ...
                UnityEngine::Quaternion deltaRotation = UnityEngine::Quaternion::AngleAxis(angle, axis);
                rightSaberTransform->set_rotation(UnityEngine::Quaternion::op_Multiply(deltaRotation, rightSaberTransform->get_rotation()));
            }
            ...
            if (rightSaberReturnDuration < 0.01f) {
                rightSaberReturnDuration = 0.01f;
            }
            ...
            UnityEngine::Quaternion targetRot_world =
                UnityEngine::Quaternion::op_Multiply(rightHandTransform->get_rotation(), originalRightSaberLocalRotation);
            ...
            if (rightSaberSimulatedWorldAngularVelocity.get_sqrMagnitude() > 0.0001f && t < 0.95f) {
                ...
                UnityEngine::Quaternion deltaRotation = UnityEngine::Quaternion::AngleAxis(angle, axis);
                rightSaberTransform->set_rotation(UnityEngine::Quaternion::op_Multiply(deltaRotation, rightSaberTransform->get_rotation()));
                rightSaberTransform->set_rotation(UnityEngine::Quaternion::Slerp(rightSaberTransform->get_rotation(), targetRot_world, t * t * t));
            } else {
                rightSaberTransform->set_rotation(UnityEngine::Quaternion::Slerp(rightSaberThrowReleaseRotation, targetRot_world, t));
            }
